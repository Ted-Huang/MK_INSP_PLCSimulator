...
#define MAX_SPLIT_WINDOW		64		// Beagle 20111101
...
typedef struct _DEFECT_PIECE {
	...
	COLORREF color;
	float	scale;
	__int64	info;	// Beagle 20140724 added.
} DEFECT_PIECE;

struct _CROSSHAIR_POINT {	// Beagle 20130611 modified.
	float x;
	float y;
	COLORREF color;
};
...
private:
	...
	HCURSOR	m_glCursor;
    OGL_ROTATE m_eRotate[ MAX_SPLIT_WINDOW ];
	...
	// ok, ok_mask, fail, fail_mask, ok_text, ok_text_mask, ng_text, ng_text_mask, region segments
	...
	CPoint m_ScaleCenter;	// Beagle 20130103 added.

	GLfloat m_xTranslate[MAX_SPLIT_WINDOW];
	GLfloat m_yTranslate[MAX_SPLIT_WINDOW];
	GLfloat m_glScale;
	...
	// Split Window -- Beagle 20111101 modified.
	int	m_wsplit;
	int m_hsplit;
	int m_width[MAX_SPLIT_WINDOW];
	int m_height[MAX_SPLIT_WINDOW];
	int m_data_w[MAX_SPLIT_WINDOW];
	int	m_data_h[MAX_SPLIT_WINDOW];
	...
	// Sprites
	//	Anchor
	int m_anchors[MAX_SPLIT_WINDOW];
	int m_anchor_x[MAX_SPLIT_WINDOW][MAX_ANCHORS];
	int m_anchor_y[MAX_SPLIT_WINDOW][MAX_ANCHORS];
	...
	//int m_rect_client_no[MAX_RECTS];
	...
	RECT m_lineRect[MAX_SPLIT_WINDOW][MAX_RECTS];	// Veritcal 垂直
	int m_lineRects[MAX_SPLIT_WINDOW];
	BOOL m_bShowLineShape;
	int m_selLine[MAX_SPLIT_WINDOW];
	// added by eric at 20161217
	RECT m_lineRect_Hori[MAX_RECTS];	// Horizontal 水平
	int m_lineRects_Hori;
	...
	DEFECT_REGION	m_defectRegion[4];	// Beagle 20120508 added.
	...
	struct text_string {	// Beagle 20120703 added.
		...
		float y;
		wchar_t str[128];
	} m_textstring[MAX_SPLIT_WINDOW][64];
	int	m_num_textstring[MAX_SPLIT_WINDOW];	// Beagle 20120703 added.
...
#define	MAX_INSPECTION_MARK	10
	...
	float	m_FrameColor[32][4];	// BOOL, R, G, B
	...
	BOOL m_bHoriVertLine;	// added by eric at 20130128
	...
	void oglDrawTextString( int TexNum );
...
private:
	...
	void PostDrawScene(BOOL isPaint = FALSE);			// Beagle 30130424 added.
	...
	BOOL UpdateTexture(IMAGE *TexImg, unsigned int texture, int xoffset, int yoffset);	// Beagle 20130424 added.
	...
	void DrawLuminanceGraph(int split_id);			// Beagle 20130816
	void DrawCurve(int split_id, bool isHorizontal,
		const unsigned char *data, int len);		// Beagle 20130816
	void DrawGrid(bool isHorizontal);				// Beagle 20130816
	void DrawDashedLine(bool isHorizontal, float position);	// Beagle 20130816
...
public:
	...
	BOOL oglUpdateTexture(IMAGE *TexImg, int xoffset, int yoffset, int TexNum=0);	// Beagle 20130424 added.
    ...
    void oglClearAllAnchor(int TexNum);
	void oglGetImageFrame(int &w,int &h,int TexNum) 
	{ 
		if (TexNum>=0 && TexNum < MAX_SPLIT_WINDOW){
			w = m_width[TexNum]; h = m_height[TexNum];
		}else{
			w = m_width[0]; h = m_height[0];
		}
	};
	void oglSetImageFrame(int w, int h) {m_width[0] = w; m_height[0] = h;};	// Beagle 20111101 modified.
	void oglSetLuminanceData(int type, unsigned char *lData=NULL,
		unsigned char *rData=NULL, unsigned char *gData=NULL,
		unsigned char *bData=NULL, int length=0, int x=-1, int y=-1);	// Beagle 20130816 modified.
	void oglProjectPoint(int& mouseX, int& mouseY, int imageX, int imageY);
	void oglTranslatePoint(int mouseX, int mouseY, int& imageX, int& imageY,
		int *window=NULL, int *winX=NULL, int *winY=NULL); // Beagle 20111101 modified.
	...
	int	 oglGetNumAnchors(int TexNum=0) { return m_anchors[TexNum]; };
	void oglGetAnchor(int num, int &x, int &y, int TexNum=0);
	void oglDeleteAnchor(int index,int TexNum=0);
	void oglAddAnchor(int x, int y,int TexNum=0);
    ...
    void oglPrintf(int TexNum, int align, float x, float y, wchar_t *fmt, ...);
	...
	// added by amike at 20120907
	void oglSetRectInfo(int index, oglShape shape, int left, int top, int right, int bottom, COLORREF color, int client_no, int TexNum=0);
	...
	void oglSetAnchorMark(int idx, int TexNum);
	...
	void oglReleaseRegionList() {		
		...
		m_region.clear();
		m_regions=0;	// added by eric at 20151001
	};
	...
	void oglSetUserLayoutRectangle(int split_id, RECT RcUserLayout, BOOL redraw=FALSE, BOOL bFocus=FALSE);	// added by eric at 20161007 function for layout user rect
	...
	void oglSetBarcodeDigitImage(int n, int x, int y, unsigned char *ptr, int depth=3);
    ...
    void oglClearLineShape( const int TexNum );
	void oglSetLineShape(int index, int left, int top, int right, int bottom, BOOL redraw=TRUE, BOOL bVeri=TRUE);	// added by eric at 20121121
	void oglGetLineShape(int index, int &left, int &top, int &right, int &bottom, BOOL bVeri=TRUE);	// added by eric at 20121121
	...
	void oglSetLineMark(int idx,int TexNum = 0);	// added by eric at 20121121
	void oglSetLineMark(CRect rt);	// added by eric at 20130107
	void oglAddLine(RECT rcLine, const int TexNum = 0);
	int	 oglGetLineNumbers() {return m_lineRects[NULL];};	// added by eric at 20121122
	...
	void oglAddCrosshair(int split, float x, float y, COLORREF color);		// Beagle 20130611 modified.
	void oglAddCrosshairPair(int split, float x1, float y1, float x2, float y2, COLORREF color);	// Beagle 20130611 modified.
	void oglAddCrosshairPair_AndDis(int split, float x1, float y1, float x2, float y2, COLORREF color,float fXDis,float fYDis,float fLDis);
	...
	void oglEnableRollingMode( const BOOL bEnable, const int iRollingMilliSeconds, const int TexNum = 0 );
    void oglRunRollingMode( const BOOL bRun, const int TexNum = 0 );

    void oglSetSingleMode( const BOOL bSingleMode ) { m_bSingleMode = bSingleMode; }
    void oglSetRotate( const int split_id, const OGL_ROTATE eRotate ) { m_eRotate[ split_id ] = eRotate; }
    void oglEnableDBclickFocusInSplitMode( const BOOL bEnable ) { m_EnableDBclickFocusInSplitMode = bEnable; }
...
private:
	// GDI objects
	CPen m_redPen;	// Beagle 20130424 added.
	CPen m_bluePen;	// Beagle 20130424 added.
// oglEnableDBclickFocusInSplitMode
    BOOL m_EnableDBclickFocusInSplitMode;
    int  m_iOrig_wsplit;
    int  m_iOrig_hsplit;
    int  m_iFocusSplitX;
    int  m_iFocusSplitY;
